...
- `bun run typecheck` **cannot run on a 4 GB box** (SIGKILL). Verified instead with scoped `tsc -p`
  runs against the app's real `tsconfig`, after generating the gitignored `routeTree.gen.ts`. Zero
  errors in changed files; the residue is pre-existing unbuilt-`@frak-labs/core-sdk` resolution.
- The test suite is **already red on `HEAD`**: ~1400 failures from `React.act is not a function`
  (react-dom 19 vs `@testing-library/react` 16 in this environment), plus all 20 `apps/listener`
  files failing to import (`No such built-in module: node:`). Not caused by this work and not fixed
  by `bun install`.
- Regressions were therefore measured by **diffing per-test JSON results against `HEAD`**:
  **0 regressed**, −14 / +11 tests (the intentional `getUpdatedToken` → `resolveSelectedToken` swap
  plus new coverage). `apps/listener` has no executing tests at all, so `extractDomain`'s behaviour
  was verified by running the function directly under Bun.
