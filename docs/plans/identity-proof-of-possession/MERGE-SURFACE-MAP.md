...
| **M8** | `PurchaseLinkingOrchestrator.claimPurchase` — the `merge:true` arms | anon → purchase's group | none live | `orchestration/PurchaseLinkingOrchestrator.ts:74`, arms at `:91-95`, `:130` and `:162-190`; default `const merge = params.merge ?? true` at `:87` | **Removed by deletion at T1.3 — not left unreachable-by-default.** Kept as a row for continuity and historical reference: this *was* a merge point, and after T1.3 it is not one. Today the only live caller passes `merge:false` (`api/user/track/purchase.ts:67`), the *default* is the merging one, `rebindExisting: merge` at `:130` rides on the same flag, and the doc block at `:14-21` names `PurchaseWebhookOrchestrator` as the default's consumer — a class that never calls this method, so the default is a default for nobody (**B2**). T1.3 deletes the parameter and all three arms, which takes `identityOrchestrator.associate` out of the purchase path entirely; no caller can name the merging behaviour afterwards because it no longer exists |
...
| **M8** | claiming group from `x-frak-client-id` | **— nothing —**, plus bearer knowledge of `(merchantId, orderId, purchaseToken)` | existing purchase's group | order/token pair — the schema states this *is* the model (`domain/purchases/db/schema.ts:100`) | none | **YES** today, and only if a caller ever passes `merge:true` — **deleted at T1.3**, after which no such call shape exists |
...
| M8 | **YES** (latent) | **YES** | **no** — T1.3 deletes the `merge` param and its three arms outright, so the merging shape is unrepresentable rather than merely unused, and `IdentityOrchestrator.associate` is unreachable from the purchase path |
...
| **G16** | M8 | `const merge = params.merge ?? true` — the safe behaviour rests on one call site remembering `merge:false` | Low-Med | untracked → **T1.3** (= **B2**) | **Delete the parameter and the arms.** Making it required is rejected: a required param still lets a future caller pass `true` and reopen the merge path, while deletion makes the behaviour unrepresentable. `rebindExisting` becomes a literal `false`, `ClaimPurchaseResult.merged` and the `/track/purchase` response's `merged` field go with it (regenerate `services/backend/user-openapi.json` in the same change — **not** the `required` `merged` on `/merge/execute`), and `associate` leaves the purchase path entirely |
...
| **B2** | M8 | `const merge = params.merge ?? true` (`PurchaseLinkingOrchestrator.ts:87`) is a dangerous default with **zero** callers relying on it — `PurchaseWebhookOrchestrator`, the class its doc block at `:14-21` names, never calls `claimPurchase` at all. The three `merge:true` arms are three distinct takeover primitives: group merge from an unauthenticated route (`:91-95`), last-claim-wins rebinding (`:130` → `PurchaseClaimRepository.ts:38-50`), and theft of an already-attributed purchase (`:162-190`) | Med now, **Critical on first mistake** | **T1.3** (= G16) | **Delete all three, rather than leaving them dead-but-typed.** `merge` comes out of `claimPurchase` and `reconcileWithExistingPurchase`, the `resolveAndAssociate` branch and the `associate` branch come out with it, and `rebindExisting` becomes a literal `false` so first-claim-wins is structural. Zero production behaviour change — the one live caller already passes `merge:false` — and the footgun becomes unrepresentable instead of merely unused: a required param still lets the next caller pass `true` |
...
- M8's "unreachable today" rests on a call-site grep: `claimPurchase` has one production caller and it
  passes `merge:false`; the other five hits are the unit test. A second caller would reopen the row
  silently — which is why T1.3 deletes the parameter and the arms rather than requiring the parameter.
  After it, the row does not rest on a grep at all: there is no merging arm left to reach.
