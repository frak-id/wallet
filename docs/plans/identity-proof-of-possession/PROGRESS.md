...
| **C** — Shopify checkout-token work (Gate 2 Phase A) | **done, fully verified; blocked on DB2** — see §3 | this commit |
| **D** — the plumbing and the provable flips | **not started** | — |
...
**T2.8 (`weightCache` invalidation, G25/AID-010) was deferred from bucket A to bucket D**,
together with its AID-011 prerequisite. Its UX-neutrality proof is conditional on mounting
the conflict surface on the standalone `/install` entrypoint, which pulls a component into
the light bundle policed by `assertEagerBundleBudget` — a bundle-budget decision, not
bucket-A plumbing. **It is still owed.**
...
## 4. What remains — bucket D

Nothing in bucket D waits on the legacy population; each row ships on its own prerequisites.
Ordering inside the bucket is real even though calendar distance is not.

1. **T2.3** — merge-token consent plumbing (§2.1). SDK signs an empty-binding `frak-merge-v1`
   and carries it on `resolved-config` as `proofs.mergeSource`, re-pushed on
   `visibilitychange`; `proofs.merge` → `proofs.mergeExecute`; listener mints the token
   **lazily**, only when an embed or the toast displays. No `ProofOp`, no window change, no
   golden-fixture regeneration, **no backend deploy**. The listener must accept **both** proof
   key shapes for one deploy window, and the rename is therefore two separate merges.
2. **T3.11** — listener refuses proofless `initiate`. After T2.3, before T3.1a.
3. **T3.1a** — `proof` required on `/merge/initiate`'s anon-source arm. One schema field.
4. **T3.2** — `/identity/ensure`, both arms in one deploy. Delete **both** proofless doors —
   the bare variant *and* the `x-frak-client-id` header fallback. Deleting only the first
   leaves G1 fully open (AID-018). Needs the wallet release that stops forwarding `a=` plus a
   7-day queue drain first.
5. **T3.3** — `install-code/generate` becomes strict (bucket C left it purely additive). The
   codeless-CTA wallet release must ship before or with it, never after, and Gate 2's
   claim-age bound must land in the same release — the moment this route requires a
   credential, the forgeable claim arm becomes the new weakest link.
6. **T3.4, T3.5, T2.6, T3.6, T3.7, T3.8, T3.10** — see the plan's bucket-D table.
7. **T2.8 + AID-011**, carried over from bucket A (§1 above).
...
4. **`PurchaseClaimRepository.upsert`'s `rebindExisting`** is now dead: after T1.3 its one
   ...
   layer down.
5. **`purchase_claims.created_at` is nullable** (`defaultNow()` without `.notNull()`), so the
   claim-age bound that T3.3 depends on needs a backfill first.
