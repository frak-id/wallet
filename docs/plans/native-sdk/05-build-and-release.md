...
**State of play: gate wired, dumps committed, `apiCheck` in CI.** BCV was wired once before, along with dumps, and both were removed deliberately — committing a dump *ratifies* the public shape, and the shape was undecided. It is decided now, and both `frak-sdk/api/frak-sdk.api` and `frak-sdk-ui/api/frak-sdk-ui.api` are in the tree. The wiring is hand-rolled because BCV registers nothing for an AGP 9 Android library and its replacement is unavailable for the same reason; [`09-android-api-surface.md`](./09-android-api-surface.md) §5a is the record. `explicitApi()` only forces you to *write* `public`; it detects nothing about a change to an already-public symbol — which is the whole point of the dump, and A4/A5 is the first change to have gone through it: two new arms and a `Kind` getter showed up in the diff, along with the sealed constructor's synthetic bridge silently gaining a parameter, which is exactly the class of change nothing else would have surfaced. Regenerate with `bun run --cwd sdk/android apiDump` (JDK 17 + `ANDROID_HOME`) and read the diff before committing it.
...
**Q3 — iOS: `init(from:)` is public API. Closed.** Filed against two inits in `FrakResolvedConfig.swift`; the actual surface was **seventeen** public `Decodable` conformances, since the row named the hand-written initializers and missed every synthesized one. Fixed as prescribed — the conformance moved to wire types — with three details worth keeping. The wire types are `private`, not `internal`: each decoder file has exactly one entry point (`ResolvedConfigDecoder.decode`, `RewardsDecoder.decode`) and nothing else needs to see them, so file scope is both available and stronger. The forgiving initializers this row said "must stay" did stay, verbatim, only now writing into a public memberwise `init` instead of `self` — and every one of the tolerance policies they encode (per-entry `placements`, `ForgivingArray`, `decodeForgiving` vs `decodeForgivingObject`, empty-folds-to-nil on `matchedProducts`) was preserved rather than reinvented; the 386-test suite passed unchanged, which is the evidence, since `ResolvedConfigDecoderTests` and `RewardsDecoderTests` exist to pin exactly those policies. **Two conformances are deliberately left**: `FrakCurrency` and `FrakLanguage` are `String`-backed enums whose decoding is just `rawValue`, which is already public *and* already the wire form the SDK sends on requests, so the conformance freezes nothing that isn't frozen — the exception is recorded in `FrakConfig.swift` so it isn't "tidied" either way later. Swift still has no ABI dump to catch a conformance creeping back, so `PublicSurfaceTests.readModelsAreNotDecodable` asserts it at runtime over all sixteen read models, with a positive control on `FrakCurrency` so the check cannot pass vacuously (cf. 8.3).
...
| Q1 `$default` freeze | answered — `09` §1/§3; all five steps landed, dumps committed |
| Q2 `@InternalFrakApi` vs promote | answered — `09` §3; marker exists, applied to `PercentEncoding` only |
| Q3 public `init(from:)` on iOS | **closed** — 17 conformances moved to private wire types; 2 recorded exceptions |
