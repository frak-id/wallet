...
**Scorecard:** 119 findings tracked historically across both audits. §1–§3 row counts, mechanically counted with `awk` over the table rows and cross-checked against the section breakdown: **44 rows total** = **9 partial** + **32 fully open** + **3 recorded as closed in-table** (S6a, S6b, S9). The 32 open split **5 in §1** (A1, A3, A4, S6c, B4), **3 in §2**, **24 in §3** (4+2+7+2+3+6 across §3.1–§3.6). Per section, rows: §1 7, §2 3, §3.1 8, §3.2 3, §3.3 8, §3.4 3, §3.5 4, §3.6 8. *A prior revision of this line said 43/25/4 and did not add up — which is exactly what it accused an earlier revision of, so the arithmetic is now shown.* The S6/C7 pass is why the open count went **up** while a §1 blocker went down: one row closed into two, and split off three genuinely-open rows (S6c, S10, S11) it had been hiding — see §0 lesson 7. §4 lists ids as closed in prose rather than a table, so an exact closed-count is still not derivable from this document without re-parsing every bolded id in §4's prose against §1–§3's rows; at least one (**S5**) is a cross-reference to a row that is still *Partial* in §3.1, and another (**B3/1.2**) to an open §2 deferral. 119 is the historical denominator only.

**This revision (S6/C7) closes one §1 blocker into two rows, S6a and S6b, and files three that it
was concealing** — S6c (the backend has no `clientId`-keyed DSR endpoint; the erasure story stops
at the device), S10 (consent durability, opposite defects per platform) and S11 (the sharing
WebView URL is identity-gated, not consent-gated). It also lands S9, ungating config and rewards.
See §0 lesson 7.
...
half without anyone updating this row. And the two remaining quarters, once separated, turned out to
be a *different severity and a different repo*: `shutdown()` is test infrastructure, not a legal
requirement, and the erasure obligation the row invoked actually lives in `services/backend`, which
has no `clientId`-keyed DSR endpoint and no owner for one. Splitting the row is what surfaced that;
six revisions of leaving it fused is what hid it. **Before implementing a row, re-derive each clause
against the code separately, and expect the count of open rows to go up.**
...
| S6c / C7 | **Still open, and it is the half that actually blocks a GDPR story.** Everything above stops at the device. There is no `clientId`-keyed erasure or access endpoint in `services/backend`, so a merchant who receives an Art-17 request can stop collection and rotate the device id but cannot delete what Frak already holds. **Mis-filed against `sdk/*` for six documents** — it is a backend row and has no owner | `services/backend/src/domain/identity/` — no DSR route |
