...
| ~~P8~~ | ~~**24 `Intl.NumberFormat` per sheet open**~~ | ~~`CampaignDetailsSheet/parts.tsx:20-52`~~ | **Won't fix — measured wrong.** 6 × 4 call sites is right; ~1–3 ms is not. Measured **0.07 ms** warm for 24 constructions; the 6.35 ms cold reading is one-time ICU init per tab, not per open | — |
...
| ~~X5~~ | ~~**Wallet has one error boundary**~~ | **Fixed** — see §6.2 #3. One shared boundary inside `AppShell` covers the four shell layouts, `_wallet` takes a route-level fallback, and `ModalOutlet`'s `Suspense` closes the modal and toasts. X8 (the business copy-paste) is untouched and still open | — |
...
| ~~**3**~~ | ~~`key={index}` over a removable field array~~ | **Fixed** (`20b6dcc6a`). Stable row keys — but the visible defect was not the one filed: values rendered correctly all along, and what actually broke was caret/focus placement after a delete | — |
| ~~**4**~~ | ~~Campaign wizard territory field is keyboard-unreachable~~ | **Fixed** (`3107985b9`). Explicit combobox semantics on the `<div>`; it cannot become a `<button>` because it nests its own chip-remove and clear-all buttons | — |
...
| ~~**6**~~ | ~~`FieldError` is silent + 11 dangling `aria-describedby`~~ | **Fixed** (`09202f99e`, `989a4db66`). Both halves were real and the second was understated: the dangling id was on **55** controls, not 11, because the app's only `FormDescription` renders outside `FormItem` and its id never resolved — so every form lost its accessible description, not just tidiness. `role="alert"` on `FieldError`, message id only on `FormControl` | — |
...
| ~~**9**~~ | ~~Client never validates `percent ≤ 100`~~ | **Fixed** (`483dd016c`). Note the gap is narrowed, not closed: `splitMatchesPool` validates only the sum, so a negative counterpart still admits an out-of-range share | — |
...
| ~~**12**~~ | ~~24 `Intl.NumberFormat` per sheet open~~ | **Won't fix — measured wrong.** The count is right (6 formatters × 4 call sites) but the cost is not: 24 constructions measure **0.07 ms** warm, against the claimed 1–3 ms. The 6.35 ms cold figure is first-call ICU init, paid once per tab, not per sheet open. `getNumberFormat` would save nothing observable | — |
...
| ~~2~~ | ~~Delete `semanticDark` (or wire up a theme switch)~~ | — | **Disabled** — emission removed, tokens kept for the planned theme |
| ~~3~~ | ~~`GlassButton` focus-visible ring~~ | — | **Done**, with a light-suite spec |
