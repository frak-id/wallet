...
**Zero of 27 findings are closed.** No commit in `cd50f13f8..5f7c52f33` touched any finding's path with intent to fix it; the SDKs moved to `1.0.0-beta.2` around them. What changed is the grading: the money-path premise behind NSD-2/3/4/6 does not hold, two coverage findings are worse than they were graded, one hygiene item was wrong when written. Severity and priority in the table below are the **2026-08-15 values**, left so cross-references resolve; the appended status column carries the 2026-09-04 read and the re-grades, with the reasoning in [Re-verification 2026-09-04](#re-verification-2026-09-04).
...
| NSD-12 | Medium | P1-next | Test coverage | The entire UIKit half of `FrakSDKUI` is compiled by tests and executed by none | **Open · re-graded High / P1-next.** Confirmed at HEAD and in `.github/workflows/apps.yaml`; the UIKit surface has grown by four suites since and executes nowhere |
...
| NSD-17 | Medium | P1-next | Doc drift | `contract.md` §1.1 documents a `/sharing` param set that no longer exists | **Open · re-graded High / P1-next** as a process failure: §1.1 was edited nine times after the audit and every phantom param survived. Generate it from `SHARING_PARAMS`; `lng` bypasses the table and must be special-cased |
...
**Two coverage findings are worse than graded.** NSD-12 → **High / P1-next**: `scripts/run.sh:do_test` and `.github/workflows/apps.yaml` both compile the suites at the simulator triple and then run bare `swift test` on the host, where `#if canImport(UIKit)` is false; four suites were added behind that guard after the audit (`13179e037`, `3dd6844a9`) and none has ever executed. NSD-17 → **High / P1-next** as a process finding: `contract.md` §1.1 was edited nine times after the audit and every phantom param (`attribution`, `native=1`, `preload=1`, `confirmed=1`, `r=`, `sdkv=`) survived every edit while correct rows were added beside them. Hand-editing has been demonstrated not to work; generate §1.1 from `SHARING_PARAMS`, and special-case `lng`, which i18next reads from the query string without passing through `table.ts`.
...
**Corrected order.** (1) NSD-1a `setPackage` — still the best S-effort security fix in this report. (2) NSD-2 + NSD-3 as one reconcile-boolean change, NSD-4's log line. (3) NSD-12: run the UIKit suites on a simulator in CI. (4) NSD-17: generate §1.1. (5) NSD-7/8 ABI decisions, unchanged clock. (6) NSD-14, NSD-5/20, NSD-18, NSD-9/10. (7) The low sweep plus the iOS twins of NSD-4/23/25.
