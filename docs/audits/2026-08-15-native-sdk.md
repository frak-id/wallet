...
The accepted risk does not stand alone. NSD-1b leaks an `anonymousId` (alongside a signed install proof) to whatever app claims the scheme, and **a leaked `anonymousId` is sufficient on its own today** — see AID-001 and AID-020 in [`2026-08-15-anonymous-id-proof.md`](./2026-08-15-anonymous-id-proof.md). Accepting NSD-1b therefore **raises** the value of closing those, which is the cheaper side of the pair by a wide margin. The remedy is not a native-side change and is not described here: it lives in [`../plans/identity-proof-of-possession/MERGE-ADMISSION-PLAN.md`](../plans/identity-proof-of-possession/MERGE-ADMISSION-PLAN.md), which is authoritative. What matters for this report is only that the dependency is real and one-directional.
...
|---|---|---|---|---|
| NSD-1a | Medium | P1-next | Identity / install handoff | Android: the Play Store rung is an unpinned `ACTION_VIEW` carrying merchant id, anonymous id and the signed install proof in the referrer |
| NSD-1b | Medium | **Accepted-risk** | Identity / install handoff | iOS: the custom-scheme rung hands the same triple to whatever app claims `frakwallet://` — unpinnable without universal links; **accepted**, see body for residual risk |
| NSD-2 | High | **P0-now** | Tracking reliability | `EventQueue.reconcile` discards its durability result; a failed compaction silently re-POSTs the whole backlog forever |
| NSD-3 | High | **P0-now** | Tracking reliability | iOS reconciles once after the entire backlog; Android checkpoints every 20 rows — 50× replay window on the platform with no idempotency key |
| NSD-4 | High | **P0-now** | Identity / tracking | Queued events dropped on a client-id change with no log, including on a spontaneous keystore rotation |
| NSD-5 | High | P1-next | Config decoding | One bad `translations` value wipes every translation on iOS; Android drops only that key |
| NSD-6 | High | **P0-now** | Wire contract | Android `products=` encoder saturates integral doubles ≥ 2^63 to `Long.MAX_VALUE` — a wrong value, not a formatting difference |
| NSD-7 | Medium | P1-next | ABI / packaging | `Composer` is in the frozen ABI dump while Compose is `implementation`-scoped — merchant compile failure |
| NSD-8 | Medium | P1-next | ABI / Java interop | `*Async` `CompletableFuture` twins complete on the main looper; `get()` from Main is a guaranteed ANR |
| NSD-9 | Medium | P2-when-picked-up | iOS sheet lifecycle | Install probe can start after `release()` and never stops; unbounded `canOpenURL` poll on MainActor |
| NSD-10 | Medium | P2-when-picked-up | iOS install flow | Store fallback unreachable in the one case that triggers it; `.installStarted` reported when nothing opened |
| NSD-11 | Medium | P2-when-picked-up | Privacy / iOS | Sharing WebView uses the app-wide persistent `WKWebsiteDataStore`; nothing ever clears it |
| NSD-12 | Medium | P1-next | Test coverage | The entire UIKit half of `FrakSDKUI` is compiled by tests and executed by none |
| NSD-13 | Medium | P2-when-picked-up | ABI gate | ABI gate is descriptor-only: nullability, equality, failure tier and `@InternalFrakApi` bytecode all pass unnoticed |
| NSD-14 | Medium | P1-next | Android lifecycle | `DeepLinkHandling.Automatic` never subscribes `OnNewIntentProvider` for the Activity that initialised the SDK |
| NSD-15 | Medium | P2-when-picked-up | Android compose | Empty `onDispose` orphans a live sheet on navigation away from the destination |
| NSD-16 | Medium | P2-when-picked-up | Test coverage | The 67-entry golden rewards corpus is loaded by neither SDK; both loader constants are dead |
| NSD-17 | Medium | P1-next | Doc drift | `contract.md` §1.1 documents a `/sharing` param set that no longer exists |
| NSD-18 | Medium | P1-next | iOS identity | `resetAnonymousId()` doc contradicts shipped behaviour; read-back consults the memo, not the disk |
| NSD-19 | Medium | P2-when-picked-up | Wire contract | The `/install` probe fragment is an iOS-only extension of a shared contract, specified nowhere |
| NSD-20 | Medium | P1-next | Config decoding | Android placement tolerance holds only by accident; the next `require*` re-opens the fixed bug |
| NSD-21 | Low | P2-when-picked-up | Wire contract | Non-canonical base64url: both natives reject, TypeScript accepts |
| NSD-22 | Low | P2-when-picked-up | Both / WebView | `isSameOrigin` treats `https://host` and `https://host:443` as different origins |
| NSD-23 | Low | P2-when-picked-up | Android tracking | Custom-payload truncation is nondeterministic (`HashMap` iteration order) and untested |
| NSD-24 | Low | P2-when-picked-up | iOS reward decoding | No finiteness guard on reward doubles; Android throws, iOS may surface `±Infinity` |
| NSD-25 | Low | P2-when-picked-up | Android identity | `IdentityMerge` claim sets are unbounded for the process lifetime |
| NSD-26 | Low | P2-when-picked-up | Publishing | The SwiftPM mirror ships a manifest invalid for its own payload |
| NSD-27 | Low | P2-when-picked-up | Hygiene | Duplicated `MainThreadDispatcher`; `@Throws` on 1 of 4 throwing entry points; history-narrating XML comment |
...
**Composition with the identity scope (this is the part that changes the arithmetic).** The residual risk above assumes the leaked `anonymousId` is only useful together with the leaked proof. Today it is not: **AID-001** and **AID-020** in [`2026-08-15-anonymous-id-proof.md`](./2026-08-15-anonymous-id-proof.md) both let a caller act on an `anonymousId` it holds no key for, so a leaked id is **sufficient on its own** — no proof, and no 30-day window. Accepting NSD-1b consequently raises the value of closing them. The fix is entirely platform-side and is specified in [`../plans/identity-proof-of-possession/MERGE-ADMISSION-PLAN.md`](../plans/identity-proof-of-possession/MERGE-ADMISSION-PLAN.md); no native change is involved, and nothing in this report should be read as describing it.
...
**NSD-27 — Hygiene cluster.** (a) `MainThreadDispatcher` exists verbatim twice, in `frak-sdk/core/` and at the bottom of `frak-sdk-ui/SharingHost.kt`; promote the core one to `@InternalFrakApi public` and delete the copy. (b) `@Throws(FrakError::class)` is on `AppLinkApi.installPageUrl` but not on `ConfigApi.resolve`, `RewardsApi.campaigns`/`best` or `SharingApi.buildLink`, all documented as throwing — inert in practice but baked into a frozen surface; pick one convention before 1.0.0. (c) `frak-sdk/src/main/AndroidManifest.xml` narrates history ("It used to ship `frak_data_extraction_rules.xml` … both were removed because…"), which the root `AGENTS.md` bans; `scripts/check-comments.ts` globs only `.kt`/`.swift`, so XML escapes the gate. (d) `Frak.shutdown` is the only non-`@JvmStatic` member of the `Frak` object. (e) iOS falls back to a shared `frak-app` return scheme when `Bundle.main.bundleIdentifier` is nil; not an interception hole, since the navigation is intercepted inside the SDK's own `WKWebView`, but worth a warning at `initialize`. (f) `TrackingConsent`'s doc says consent is "Stored in the identity `KeyValueStore` suite", but `Frak.initialize` passes `UserDefaultsStore(suiteName: .consentSuiteName)` (`id.frak.sdk.consent`) while identity is now `FileKeyValueStore` — a load-bearing distinction, since consent must survive a restore and identity must not. (g) Android cross-origin sub-frame navigations are cancelled with no log even in a debuggable build; and where iOS explicitly cancels a sub-frame `returnScheme` navigation, Android relies on `WebView` not starting Activities for unhandled schemes — same net behaviour, invariant stated on only one platform.
...
6. **Take the ABI decisions before anything consumes the published artifact.** `api(libs.compose.runtime)` or a third artifact; decide futures-vs-callbacks; settle `@Throws` consistency. These are free now and breaking later. Closes NSD-7, NSD-8, NSD-27(b). **M**
...
13. **Strengthen the ABI gate with a nullability/equality companion test, and correct the README's `@Target` sentence.** Closes NSD-13, NSD-27 partial. **M**
