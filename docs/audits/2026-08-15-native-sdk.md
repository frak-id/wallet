...
| NSD-1b | Medium | **Accepted-risk** | Identity / install handoff | iOS: the custom-scheme rung hands the same triple to whatever app claims `frakwallet://` — unpinnable without universal links; **accepted**, see body for residual risk. **Open.** The AID-001/AID-020 composition below is moot; both are closed |
| NSD-2 | High → Medium | **P0-now** → P1-next | Tracking reliability | `EventQueue.reconcile` discards its durability result; a failed compaction silently re-POSTs the whole backlog forever. **Open.** The premise was wrong when written: arrival is idempotent server-side (see re-verification). Reliability defect, not payout |
| NSD-3 | High → Medium | **P0-now** → P2-when-picked-up | Tracking reliability | iOS reconciles once after the entire backlog; Android checkpoints every 20 rows — 50× replay window on the platform with no idempotency key. **Open.** Same idempotency argument; single reconcile after the loop confirmed at HEAD |
| NSD-4 | High → Medium | **P0-now** → P1-next | Identity / tracking | Queued events dropped on a client-id change with no log, including on a spontaneous keystore rotation. **Open.** The re-grade is P1-next for the log line, P2 for the load split. iOS has the same silent drop and mint-on-corrupt (`EventOutbox.swift`, `DeviceKey.swift`) |
| NSD-5 | High → Medium | P1-next → P2-when-picked-up | Config decoding | One bad `translations` value wipes every translation on iOS; Android drops only that key. **Open.** Needs a non-string through two typed schemas and response validation; hardening |
| NSD-6 | High → Low | **P0-now** → P2-when-picked-up | Wire contract | Android `products=` encoder saturates integral doubles ≥ 2^63 to `Long.MAX_VALUE` — a wrong value, not a formatting difference. **Open.** Saturation reproduced (`1e19`), but needs an integral price ≥ 2^63; the Kotlin golden tests this body says are missing exist (`RewardRepositoryTest`) |
...
| NSD-24 | Low → Informational | P2-when-picked-up | iOS reward decoding | No finiteness guard on reward doubles; Android throws, iOS may surface `±Infinity`. **Open.** Refuted on Swift-native Foundation (`1e999` → `dataCorrupted`); unconfirmed only on the iOS 15–17 ObjC path. Add the fixture and close |
