...
**The backend on this branch must not be deployed to a stage whose database has not taken the `purchase_items` change.** See [The migration](#the-migration): `local` and `dev` ship on this branch, `prod` does not.
...
| `frakContext.ts` (in-code) | `update` "returns null on failure" | Now true: `parse`, `update` and `remove` all go through one `safeParseUrl` guard | **fixed** |
...
1. **Take the `purchase_items` migration through QA on `dev`, then generate `prod`.** `local` (`0040_chubby_calypso`) and `dev` (`0044_typical_alex_wilder`) ship on this branch; `drizzle/prod` still ends at `0020` and nothing in CI blocks a `main` merge on it. The backend writes `total_price` and uses the new constraint as its `ON CONFLICT` arbiter, so on prod every purchase webhook with items fails until the migration lands. The DDL, safety argument and lock-window note are in `docs/plans/purchase-items-line-key-migration.md`. Owned by the DB team. Also decide the unrelated pending prod drift the generation surfaced (`install_codes` columns and check constraint, a `purchase_claims` index).
...
PSC-2 and PSC-3 need a `purchase_items` change (nullable `total_price`, and item identity re-keyed to `UNIQUE NULLS NOT DISTINCT(purchase_id, external_id, sku)`). The `schema.ts` declaration and the `local` (`0040_chubby_calypso`) and `dev` (`0044_typical_alex_wilder`) migrations ship on this branch; **`prod` does not**. Both shipped files are the same three statements (drop the old index, add `total_price`, add the constraint), their snapshots chain correctly, and no dedupe step is needed: the old key `(external_id, purchase_id)` is a subset of the new one. The DDL, the safety argument, the lock-window note and the remaining rollout are in `docs/plans/purchase-items-line-key-migration.md`.

**The backend must not be deployed to a stage whose database has not taken that change** — the code writes `total_price` and uses the constraint as its `ON CONFLICT` arbiter. Today that means `prod`.
...
- **Nothing here ran against a live Postgres or a live store.** The DDL, the reconciliation SQL and every PHP change are reviewed by eye and by rendered-SQL assertions only. Applying the `local` migration is the first real execution.
- **PrestaShop `totalPrice` does not see vouchers.** `order_detail.total_price_tax_incl` is the line after specific prices; cart rules live in `order_cart_rule` and are never allocated per line, so a matched-basis percentage on a voucher order pays on the pre-voucher line sum, bounded by the `purchase.amount` clamp. Allocating `order_cart_rule` pro-rata is the fix if that matters; verify against a captured voucher payload first.
- **An empty delivery on the claim path yields an empty interaction.** When a custom or Magento redelivery omits `items` and a claim is present, `upsertWithItems` leaves the stored lines alone but returns `[]`, and the interaction is created idempotently with no items — `scope_matched_no_item`, no product-scoped reward, permanently — although the late-claim path would have read the stored rows. Pre-existing ordering, now cheap to fix: return the stored rows when the delivery carries none.
- **A stale product pick survives a fragment re-activation.** `useSharingPageController` keeps `pickedProductIndex` across a new `products` list on the same mounted view; with hidden sku-only entries now possible, a stale index can select one. Validate the pick with `isRenderableProduct` before honouring it.
