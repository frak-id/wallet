...
8. **SDK legacy property casing** — the SDK event map (`sdk/core/src/utils/analytics/events/`) mixes snake_case and camelCase in *property* names (e.g. `handshake_duration_ms` vs `sdkVersion`, `referrerClientId`, `walletStatus`). The camelCase fields are preserved from the pre-typed `FrakEvent` string union that predates this plan, so that OpenPanel dashboards keyed on them keep working. **Event names** are snake_case everywhere; new SDK properties MUST be snake_case; the legacy camelCase props stay only until we do a deliberate OpenPanel-side alias migration.
