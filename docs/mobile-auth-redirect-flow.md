...
- Stored in `localStorage` (cross-tab persistence for mobile app opening new browser tab)
...
export async function processAuthCallback(
    ...
    // Validate state (localStorage for cross-tab persistence)
    const savedState = localStorage.getItem("frak_auth_state");
    if (state !== savedState) {
    ...
    try {
        ...
        // Clean up
        localStorage.removeItem("frak_auth_state");
        cleanupAuthParams();
...
export function SsoButton({
    ...
    const handleClick = () => {
        if (isMobile()) {
            ...
            localStorage.setItem("frak_auth_state", state);  // localStorage for cross-tab persistence
...
| **Listener** | `apps/listener/app/module/component/SsoButton.tsx` | Update ✅ (inline utils) |
| **Listener** | `apps/listener/app/module/handlers/lifecycleHandler.ts` | Update ✅ (mobile-auth-complete) |
| **Shared** | `packages/wallet-shared/src/common/api/backendClient.ts` | Update ✅ (401 handling fix) |
| **Shared** | Uses existing `ua.isMobile` from `@frak-labs/wallet-shared` | N/A ✅ |
...
- [x] Use `localStorage` for CSRF state (cross-tab persistence when mobile app opens browser)

### Phase 4: Listener Update
**Status:** ✅ Completed
**Completed:** January 8, 2026 (as part of Phase 2)

- [x] Update `SsoButton` with mobile detection (`ua.isMobile`)
- [x] Create `MobileSsoButton` component for redirect flow
- [x] Create mobile auth utilities inline (`generateState`, `buildMobileLoginUrl`)
- [x] Use `postMessage` with `iframeLifecycle: "redirect"` for parent navigation
- [x] Store state in `localStorage` for CSRF protection (cross-tab persistence)
- [x] Add `mobile-auth-complete` lifecycle handler
- [x] Fix session persistence: skip 401 clearing for `/wallet/balance` and `/wallet/tokens`
- [ ] Test full flow (pending manual verification)
