...
- **The standalone build is a SECOND pass over `dist/`**: `emptyOutDir: false`, assets under `dist/standalone/`. It MUST run after the SPA build, which empties `dist/`. That ordering is why `dev:built` (`vite build --watch`) does NOT run it — under watch mode `/sharing` falls back to the SPA route. Use `dev:sandbox` (`vite dev`) to work on the standalone pages: Vite's HTML fallback serves `sharing.html` from source, with HMR.
