...
export function DiscoveryStep({
    ...
    /**
     * Single settle latch — both the local probe and the pairing's
     * `onSuccess` race for it; whichever wins first dictates the mode
     * and tears the loser's side down. Mode-specific cleanup is inline:
     *  - `local` wins → cancel any in-flight signature requests on the
     *    detached pairing, softReset the client, drop the detached
     *    session snapshot.
     *  - `remote` wins → clear any stale local-probe error so it
     *    doesn't flash on top of the next step; pairing stays alive
    ...
    useEffect(() => {
        client.initiatePairing({
            ...
            onSuccess: () => {
                ...
                settle({
                    mode: "remote",
                    targetAuthenticatorId: peerAuthId,
                    pairingId: state.pairing.id,
                });
            },
        });
    ...
    const runLocalProbe = useCallback(async () => {
        ...
        try {
            ...
            if (!targetAuthenticatorIds.includes(raw.id)) {
                throw new Error(MergeError.DiscoveryLocalUnexpectedCred);
            }
            settle({ mode: "local", targetAuthenticatorId: raw.id });
        } catch (err) {
