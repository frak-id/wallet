...
export type ResolvedPlacement = {
    ...
    components?: {
        buttonShare?: {
            ...
            noRewardText?: string;
            /**
             * Which UI the share button opens. Wire type: the backend still
             * emits `"embedded-wallet"` and `"share-modal"` for some merchant
             * configs, and the SDK routes both to the sharing page.
             */
            clickAction?: "embedded-wallet" | "share-modal" | "sharing-page";
            css?: string;
        };
        /**
         * Embedded-wallet button config. The backend still emits the whole
         * object, but `<frak-button-wallet>` only reads `position`.
         */
        buttonWallet?: {
            position?: "right" | "left";
            css?: string;
