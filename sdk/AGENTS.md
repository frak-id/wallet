...
- **There is no Android dex budget check any more.** `checkDexSizeBudget` measured unminified d8 output and was retired in `32836c217`: R8 `mapping.txt` attribution against a minified harness APK put the SDK at 60 KB of executable code against the 479 KB the gate watched. What does the real work is the empty `consumer-rules.pro` — R8 shakes out 46% of the SDK's classes in a merchant build, and a blanket keep rule would end that.
