...
| Registry | Maven Central Portal (not OSSRH — decommissioned), namespace `id.frak.sdk` claimed, publish path still blocked by A6 | SPM only, no CocoaPods. Mirrored to [`frak-id/frak-ios-sdk`](https://github.com/frak-id/frak-ios-sdk) — SwiftPM reads `Package.swift` from a repo root only, so `sdk/ios` is unreachable to a merchant. `release-ios-sdk.yml` on an `ios-v*` tag; `main` seeded, no tags yet |
