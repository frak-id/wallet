...
- **`FrakClient` is a sealed concrete class with five namespaces** (`config`, `rewards`, `sharing`, `tracking`, `appLink`), not an interface/protocol — see `docs/plans/native-sdk/09-api-shape.md`. Adding a member to it is additive on both platforms; there is no supported way to substitute a fake for it, so `frak-sdk-ui`'s tests inject narrow functions instead of a client abstraction, and merchant tests should point `FrakEnvironment.Custom`/`.custom` at a stub server.
