...
actor RewardRepository {
    ...
    func fetch(
        ...
        // already a compact, stable representation of the input.
        ...
        // about one product set: folding products in would mint a fresh key with a zero
        // failure count on every product page, so a failing backend would be re-dialled
        // instead of backed off. This is the key the repository used before products existed.
    ...
    private func request(
        ...
        // Sweep before inserting: `products` put a caller-controlled, up-to-4KB string in the
        // key, so the map is no longer bounded by the handful of merchant/currency/audience
        // combinations it used to hold. An entry past its TTL can never be served again, so
        // dropping expired entries here bounds the map to what was actually asked for inside
        // one TTL window rather than letting a browsed catalogue accumulate for the app's life.
    ...
    /// query, so two calls differing only in audience — or in products — return genuinely
    /// different answers.
