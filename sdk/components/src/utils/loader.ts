 ...
 * This function dynamically injects the loader.css file generated by the CSS modules plugin
 *
 * Note: Uses import.meta.url instead of document.currentScript because
 * document.currentScript returns null in ES modules.
 */
function loadCssModules() {
    // Check if styles are already loaded
    const cssId = "frak-components-styles";
    if (document.getElementById(cssId)) {
        return;
    }

    // Use import.meta.url to get the current module URL
    // This works in ES modules where document.currentScript is null
    const moduleUrl = import.meta.url;

    // Extract base URL (remove filename)
    const baseUrl = moduleUrl.substring(0, moduleUrl.lastIndexOf("/"));

    // Create link element
    const link = document.createElement("link");
    link.id = cssId;
    link.rel = "stylesheet";
    link.href = `${baseUrl}/loader.css`;

    // Append to head
    document.head.appendChild(link);
}

// Load CSS before initializing SDK
loadCssModules();
