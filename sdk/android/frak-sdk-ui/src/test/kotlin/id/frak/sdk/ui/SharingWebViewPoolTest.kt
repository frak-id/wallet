...
class SharingWebViewPoolTest {
    ...
    fun `warming loads the wallet origin before any sheet exists`() {
        ...
        assertEquals(
            ...
            "$WALLET_ORIGIN/sharing?native=1&sid=warm",
    ...
    fun `the warm load finishing after the sheet took the view is not the sheet's page`() {
        val pool = pool(preload = true)
        pool.warm() // onPageStarted for the warm URL has fired; onPageFinished has not
        var ready = 0
        var visible = 0
        var failed = 0
        val handle =
        ...
        handle.client().onPageFinished(handle.view, "$WALLET_ORIGIN/sharing?native=1&sid=warm")
