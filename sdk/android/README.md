...
- **Unused AGP build features are off in two different places, and only two of them are still a `gradle.properties` flag.** `resValues` and `shaders` are `android.defaults.buildfeatures.*` there; `buildConfig` is `buildFeatures { buildConfig = false }` in `frak-publish.gradle.kts`, because the SDK version is a reviewed Kotlin constant (`FrakSdkVersion`) rather than a generated field. `aidl` and `renderscript` used to sit alongside the first two and no longer exist — AGP 9 removed the flags, warning on every invocation while doing nothing, and their default is now off regardless. Each feature left on costs a task per module per variant and can only grow the AAR.
- **`android.nonTransitiveRClass=true` is not an optimisation.** Without it this SDK's `R` class would carry every dependency's resource ids into a merchant's build.
...
- **Five files carry the version, and they are gated as a set.** `frak.sdk.version` in `gradle.properties` is the source of truth; `FrakSdkVersion.CURRENT` (which feeds the `x-frak-sdk-version` header and the `?sdkVersion=` param on `/sharing` URLs), `package.json`, the harness coordinates in `example/native-android/app/build.gradle.kts` and the `publishLocal` path in this file all follow it. `bun run --cwd sdk/android check:version` — inside `check`, `bundle` and the release workflow — compares all five and fails if `CHANGELOG.md` has no section for the version being cut. The list lives in `scripts/native-version.ts` at the monorepo root, because one of those files is outside this package. Gradle's own `checkSdkVersionMatchesArtifact` still covers the pair it can see.
