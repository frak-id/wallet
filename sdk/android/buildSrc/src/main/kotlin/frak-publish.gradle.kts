...
// Central requires a `-javadoc` artifact to exist and never opens it, so this ships a stub: AGP's
// `withJavadocJar()` runs a bundled Dokka 1.x that throws on any `sealed` type it reads as a
// binary. KDoc is not lost — `withSourcesJar()` above is what an IDE reads. The reasoning is in
// the README's "Open decisions"; an entirely empty jar reads as a build bug, so the stub says why.
val javadocStub =
