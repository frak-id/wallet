 ...
 * `getMerchantPermissions`, whose platform-admin grant is read-only (see
 * session.ts). Every handler records the acting admin's business `accountId`
 * as `createdBy` (null for the legacy-JWT grace path, which carries no
 * account row).
 */
