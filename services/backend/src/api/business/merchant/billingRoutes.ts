 ...
 * their own files (and their own per-route auth guards —
 * `requireMerchantAccess` / `getMerchantPermissions` for accounting/documents,
 * `platformAdminAuthenticated` for admin CRUD), but compose under one
 * `/:merchantId/billing` prefix so the merchant router mounts one billing
