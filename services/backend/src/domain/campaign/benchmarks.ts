/**
 * Product-config constants used by the campaign reporting layer to
 * surface Frak-vs-Meta comparisons and the platform-fee CPA split.
 *
 * These are NOT measured from production data — they exist because the
 * datasets the frontend needs (Meta Ads industry CPA, Frak's own
 * platform fee) are not tracked in Postgres today. Centralising them
 * in the campaign domain keeps the orchestrator declarative and gives
 * Product one file to edit when the numbers move.
 */
 ...
 *
 * Values are rough public benchmarks rounded to whole units — a
...
/**
 * Share of total reward spend attributed to the Frak platform fee in
 * the CPA breakdown chart. `asset_logs.recipient_type` only stores
 * `referrer` and `referee` today, so the `frak` segment is an overlay
 * derived from total spend rather than a sum of real rows. When the
 * platform fee becomes a real on-chain split, replace this with a
 * SUM on a new `recipient_type = 'platform'` filter.
 */
