...
describe("DnsCheckRepository", () => {
    ...
    describe("getNormalizedDomain", () => {
        it("should normalize domain with https prefix", () => {
            const result = dnsCheckRepository.getNormalizedDomain("https://www.example.com");
            expect(result).toBe("example.com");
        });

        it("should normalize domain without https prefix", () => {
            const result = dnsCheckRepository.getNormalizedDomain("www.example.com");
            expect(result).toBe("example.com");
        });

        it("should handle domain without www", () => {
            const result = dnsCheckRepository.getNormalizedDomain("example.com");
            expect(result).toBe("example.com");
        });

        it("should handle subdomain", () => {
            const result = dnsCheckRepository.getNormalizedDomain("https://subdomain.example.com");
            expect(result).toBe("subdomain.example.com");
        });

        it("should handle complex domain with path", () => {
            const result = dnsCheckRepository.getNormalizedDomain("https://www.example.com/path/to/page");
            expect(result).toBe("example.com");
        });
    });
    ...
    describe("getDnsTxtString", () => {
        it("should generate correct DNS TXT string", () => {
            ...
            const result = dnsCheckRepository.getDnsTxtString({ domain, owner: mockOwner });
        ...
        it("should generate different hashes for different owners", () => {
            const domain = "example.com";
            const owner2 = "0xabcdefabcdefabcdefabcdefabcdefabcdefabcd" as const;
            
            const result1 = dnsCheckRepository.getDnsTxtString({ domain, owner: mockOwner });
            const result2 = dnsCheckRepository.getDnsTxtString({ domain, owner: owner2 });
    ...
    describe("isValidDomain", () => {
        ...
        it("should return false when invalid setup code is provided", async () => {
            ...
            const result = await dnsCheckRepository.isValidDomain({
                domain: "example.com",
                owner: mockOwner,
                setupCode: "0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef",
            });
