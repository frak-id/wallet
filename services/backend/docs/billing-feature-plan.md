...
> Monthly bills are **not** admin-triggered: they are generated by a daily cron
> (`jobs/monthlyBillGeneration.ts`), so there is no `POST /monthly-bills`
> endpoint. See §6.4.
...
**Accuracy limitation (resolves feasibility §3.1):** this ledger is only as complete as the admin-entered deposits/withdrawals. On-chain movements not captured as `billing_documents` rows will skew it. A v1 on-chain divergence check (read the live bank balance at generation and flag the bill if it diverged) was implemented and then **removed** — the check is only meaningful for the just-closed month, is a soft non-blocking flag no process consumes, and added campaign-bank/on-chain-balance dependencies for unclear payoff. If reconciliation is needed later, do it properly (an indexer / balance snapshot at settlement), not a live spot-read at generation time.
...
Generated by a **daily cron** (`jobs/monthlyBillGeneration.ts` → `MonthlyBillOrchestrator.backfillAllMerchantBills`), serialized across replicas by a Postgres advisory lock. For each merchant that has complete accounting info **and** at least one deposit, it backfills every missing month from the oldest of (merchant creation, first deposit) up to — but excluding — the current (still-open) month. Generation is idempotent via the partial-unique `(merchant_id, period_start) WHERE kind='monthly_bill'` constraint → a duplicate insert 23505s and is caught (`MonthlyBillAlreadyExistsError`) and skipped; existing periods are also pre-skipped via `listMonthlyBillPeriodStarts` to avoid wasted work.

Bills are stored **data-only** (no PDF); the PDF renders lazily on the merchant's first download (same path deposit/withdraw notes now use). No on-chain reconciliation is performed (§6.2).
