...
**WebSocket upgrades are explicitly skipped** in `onRequest` (detected via the `Upgrade` header). A WS handshake never produces a normal HTTP response, so counting it would leak the in-flight gauge by +1 per connection forever — the pairing WS route (`/user/wallet/pairing/ws`) opens long-lived sockets. WS gets its own metrics separately (deferred).
