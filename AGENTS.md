...
- **Native SDK versioning is deliberately outside Changesets, and independent per platform**: `id.frak.sdk:core` and `FrakSDK` version separately — a merchant's binary freezes at store submission, so a JS-style patch cadence is meaningless, and either platform must be able to take a hotfix alone. Both `package.json` files are `private` and in `.changeset/config.json` `ignore`; they exist only to dispatch to `scripts/run.sh`. Nothing pairs the two versions; they have matched so far because they were cut together, not because anything enforces it. What Changesets would have owned is replaced by three things. **The bump is one hand-written commit that moves every site at once** — five files on Android (`gradle.properties`, `FrakSdkVersion.kt`, `package.json`, the harness coordinates in `example/native-android`, and the README twice — the merchant integration snippet and the `publishLocal` path) and three on iOS (`FrakSDKVersion.swift`, `package.json`, the `exact:` pin in `README.mirror.md`) — gated as a set by `bun run check:native-versions` (`scripts/native-version.ts`), which runs inside `bun run --cwd sdk/android check`, every iOS `run.sh` subcommand, and both release workflows. The list lives at the monorepo root because one Android site is outside that package, and because a per-platform copy is the thing that drifts. **The CHANGELOG is `sdk/{android,ios}/CHANGELOG.md`** (Keep a Changelog), written under `[Unreleased]` as you work and promoted by the release commit; the gate fails a release whose version has no section, and both workflows publish that section as the GitHub release body. The iOS one ships inside the mirror payload, where it is the only history a merchant can see. **The publish is the tag** (`android-v*` / `ios-v*`); iOS lints, builds and tests the released tree on macOS *before* the mirror push, since the mirror refuses to retag a published version, and Android drafts its GitHub release while the Portal deployment is still `USER_MANAGED` so both are published by the same hand.
...
- **Vite 8 bundles with rolldown natively** (Rust) — no `rolldown-vite` alias; `@wagmi/connectors` stubbed in resolutions to avoid MetaMask SDK bloat.
- **The browser floor is Safari 15.4**: `BROWSER_TARGET_ECMA` / `BROWSER_TARGET_SAFARI` live in `packages/dev-tooling/src/es-version.ts` (`vite.ts` re-exports them, so existing import sites are unchanged), with `BROWSER_TARGET` and the Lightning CSS targets still in `vite.ts`. Coverage is not uniform, and the gaps are deliberate: the four vite configs that pin a target read `BROWSER_TARGET`; the five `tsdown` configs keep literal targets held to the constant by `bun run check:es-targets`, which also fails on a `tsdown` config nobody registered. Four surfaces sit outside a pin — `services/backend/src/utils/processCss.ts` (lowers merchant-supplied CSS wider on purpose), the wallet service worker (`ES2020`, commented), and `apps/business` + `apps/shopify`, which run the gate without a pin because the floor comes from wallet traffic they do not serve. Never write `baseline-widely-available` — that alias moved twice (safari16 under vite 7, safari16.4 under vite 8), silently, inside unrelated migrations. Three layers enforce the floor and they catch different things: `lib: ES2022` across the ten browser-shipped tsconfigs rejects what you *author*; `assertBundleEsVersion` rejects what a vite app *ships*; and `bun run check:es-output` rejects what the `tsdown` packages ship — ambient augmentations `lib` cannot see (`bun-types` merges `Promise.withResolvers` onto `PromiseConstructor`), dependency code hidden by `skipLibCheck`, and syntax rolldown emits verbatim below target. `bun-types` cannot be removed from the four apps or `wallet-shared`: their `@backend-*` path mappings pull real backend source (`Bun.serve`, `Bun.password`, `bun:jsc`) into each app's own compilation.
