...
- **`sdk/{android,ios}` are scaffolding, not an SDK**: real buildable library projects (two artifacts each — core is UI-free, UI carries the web view), but the only real source file per platform is `FrakSdkVersion`. Everything else is empty module folders with `.gitkeep` markers naming what lands there. A green `assembleRelease` / `swift build` means the scaffolding is coherent, **not** that any behaviour exists. Plan: `docs/plans/native-sdk/`.
