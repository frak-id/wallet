...
- **The browser floor is Safari 15.4**: `BROWSER_TARGET` / `BROWSER_TARGET_ECMA` in `packages/dev-tooling/src/vite.ts`, read by every `build.target`, the Lightning CSS targets, and the `assertBundleEsVersion` gate. One known exception, deliberate and commented: `services/backend/src/utils/processCss.ts` lowers merchant-supplied CSS at its own wider targets, and importing build tooling into a runtime service is not worth the coupling. Never write `baseline-widely-available` — that alias moved twice (safari16 under vite 7, safari16.4 under vite 8), silently, inside unrelated migrations. Two layers enforce the floor and they catch different things: `lib: ES2022` across the ten browser-shipped tsconfigs rejects what you *author*, and `assertBundleEsVersion` rejects what actually *ships* — ambient augmentations `lib` cannot see (`bun-types` merges `Promise.withResolvers` onto `PromiseConstructor`), dependency code hidden by `skipLibCheck`, and syntax rolldown emits verbatim below target. `bun-types` cannot be removed from the four apps or `wallet-shared`: their `@backend-*` path mappings pull real backend source (`Bun.serve`, `Bun.password`, `bun:jsc`) into each app's own compilation.
