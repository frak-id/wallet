...
- **`sdk/ios` cannot declare Swift 6 in `Package.swift`**: `.swiftLanguageMode(.v6)` is a PackageDescription 6.0 API and the tools-version floor is 5.9, so `-swift-version 6` comes from `scripts/run.sh`. Tests use Swift Testing, not XCTest — the XCTest overlay cannot link at an iOS-simulator triple from SwiftPM.
