...
- **Comments were cut back on purpose (`f7e69f4`)**: the branch had grown multi-paragraph rationale essays, "why not X" sections and markdown tables on simple declarations; that commit took 6826 comment lines to 2275 and the longest block from 45 lines to 9. The budget it set is the standard: public surfaces get a 1-2 sentence summary plus short `@param`/`@return`, internals get a one-liner only where a real trap would otherwise be "fixed" by the next reader, tests get nothing their name already says. Comment the trap, not the code, and never the history — a reviewer reads the diff, not a narrative of what you tried. No linter enforces this, so it decays silently.
