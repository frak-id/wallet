...
export type RpcClient = {
	 ...
	 */
	request: <TMethod extends PromiseMethods>(
		method: TMethod,
		...params: RpcParameters<TMethod> extends undefined
			? []
			: [RpcParameters<TMethod>]
	 ...
	 */
	stream: <TMethod extends StreamMethods>(
		method: TMethod,
		...params: RpcParameters<TMethod> extends undefined
			? []
			: [RpcParameters<TMethod>]
	) => AsyncIterableIterator<RpcReturnType<TMethod>>;
...
export function createRpcClient(config: RpcClientConfig): RpcClient {
	...
	// Message handler
	function handleMessage(event: MessageEvent<RpcMessage>) {
		// Validate origin
		try {
	 ...
	 */
	function sendMessage(message: RpcMessage) {
	 ...
	 */
	function request<TMethod extends PromiseMethods>(
		method: TMethod,
		...params: RpcParameters<TMethod> extends undefined
			? []
			: [RpcParameters<TMethod>]
		...
		return new Promise<RpcReturnType<TMethod>>((resolve, reject) => {
			const id = generateId();
	 ...
	 */
	async function* stream<TMethod extends StreamMethods>(
		method: TMethod,
		...params: RpcParameters<TMethod> extends undefined
			? []
			: [RpcParameters<TMethod>]
		...
		const queue: RpcReturnType<TMethod>[] = [];
		let resolveNext:
			| ((value: IteratorResult<RpcReturnType<TMethod>>) => void)
			| null = null;
		let streamEnded = false;
		let streamError: Error | null = null;

		// Create stream controller
		const controller: StreamController<RpcReturnType<TMethod>> = {
			push: (value: RpcReturnType<TMethod>) => {
				if (resolveNext) {
		...
		try {
			while (!streamEnded || queue.length > 0) {
				...
				await new Promise<IteratorResult<RpcReturnType<TMethod>>>((resolve) => {
					resolveNext = resolve;
				});
			}
		} finally {
