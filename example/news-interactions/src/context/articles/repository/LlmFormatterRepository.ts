...
class LlmFormatterRepository {
    ...
    public async formatNews(
        ...
        const basePrompt = prompt
            ? `${prompt} ${news.text}`
            : `IMPORTANT: Your task is to format the text, not to rewrite or summarize it. The content must remain exactly the same, only adding markdown formatting.
