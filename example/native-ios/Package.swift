...
// lower than its dependencies' (sdk/ios needs macOS 12), so omitting it fails resolution
// rather than opting out. A bare `swift build` here still produces a macOS binary —
// `scripts/run.sh build` passes an explicit iOS simulator target instead.
//
// SwiftPM cannot build a runnable .app — Info.plist (URL schemes,
// LSApplicationQueriesSchemes) is never applied. This package exists to typecheck the
// sources in CI, not to produce an installable app.
let package = Package(
